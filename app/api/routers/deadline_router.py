...
def lauch_deadline_service():
    return service()


    
@router.get("/jobs/{job_id}", response_model=Response)
def get_job(job: Job, deadline_service: service = Depends(lauch_deadline_service)):

    return Response

@router.get("/{locate}/jobs/{job_id}")
def get_specific_job(locate: str, job_id: str, deadline_service: service = Depends(lauch_deadline_service)):
...
def read_status_jobs(locate: str, status: str, deadline_service: service = Depends(lauch_deadline_service)):
