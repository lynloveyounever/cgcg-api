...
def test_list_transfers():
    ...
    response = client.get("/api/v1/media_shuttle/transfers")
...
def test_create_and_get_transfer():
    ...
    # Create
    response_create = client.post("/api/v1/media_shuttle/transfers", json=new_transfer_data)
    ...
    # Get
    response_get = client.get(f"/api/v1/media_shuttle/transfers/{new_id}")
    assert response_get.status_code == 200
    assert response_get.json() == created_transfer

    # Cleanup (delete the created transfer)
    response_delete = client.delete(f"/api/v1/media_shuttle/transfers/{new_id}")
